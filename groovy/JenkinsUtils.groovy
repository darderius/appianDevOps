...
void requestPatchAnalysis(customProperties) {
  aquamanUrl = SITEBASEURL + "/webapi/analysePatch"
   response=sh( script:"curl --location  --request POST \"$aquamanUrl\" --header \"Appian-API-Key: $APIKEY\" --header \"Appian-Document-Name=finalPackage.zip\" --form \"json={\"packageFileName\":\"finalPackage.zip\",\"name\":\"$DEPLOYMENTNAME\"}\"", returnStdout: true).trim()
